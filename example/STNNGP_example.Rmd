...
In this example, we arbitrarily choose the first variable to be the root and 
...
starting <- list("beta" = beta, "phi" = phi, "sigma.sq" = sigma.sq, 
                 "tau.sq" = tau.sq, "rho" = rho, "adjmat" = A)
tuning <- list("phi" = 0.1, "sigma.sq" = 0.1, "tau.sq" = tau.sq/5, 
               "rho" = rep(0.05, q-1))                                          
priors <- list("phi.Unif" = c(3/1, 3/0.3), "sigma.sq.IG" = c(2, sigma.sq), 
               "tau.sq.IG" = cbind(2, tau.sq), "rho.Unif" = c(-1, 1))
...
# create coordinates on a grid 
coords <- expand.grid(seq(0, 1, length = 15), 
                      seq(0, 1, length = 15)) %>% 
  arrange(Var1) %>%                                                             
  as.matrix() 
...
for (me in 2:q) {
  parent <- which(A[,me] == 1)
  cross_phi[me-1] <- sqrt(0.5*(phi[parent]^2+phi[me]^2))
}
...
save <- 1000
n.samples <- burn + thin*save
starting <- list("beta" = beta, "phi" = phi, "sigma.sq" = sigma.sq, "tau.sq" = tau.sq, 
                "cross.phi" = cross_phi, "rho" = rho, "adjmat" = A)
tuning <- list("phi" = rep(0.1, q),                                        
               "cross.phi" = rep(0.5, q-1), 
               "sigma.sq" = rep(0.01, q), "tau.sq" = tau.sq/5, 
               "rho" = rep(0.01, q-1))
...
for (i in 1:reps) {
  ...
  cross_phis[i,] <- out$p.crossphi.samples[burn+thin*1:save,] %>% colMeans()
  rhos[i,] <- out$p.rho.samples[burn+thin*1:save,] %>% colMeans()
  telapsed[i] <- out$run.time[3]
}
```
...
params_est <- cbind(betas, tau.sqs, sigma.sqs, phis, ssqphis, cross_phis, rhos, telapsed) %>% 
  as.data.frame()
params_true <- c(c(beta), tau.sq, sigma.sq, phi, sigma.sq*phi, cross_phi, rho) %>% 
  matrix(., nrow = 1) %>% 
  as.data.frame()
col_labels <- c(paste0("beta[",1:(p*q),"]"), 
                paste0("tau[",1:q,"]^2"), 
