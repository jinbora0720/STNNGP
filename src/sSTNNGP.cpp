...
void updateBF_parent(double *B, double *F, double *c, double *C, double *coords,
  ...
  int i, k, k2, l, l2, MeIndx; // change: MeIndx added
  int info = 0;
  int inc = 1;
  double one = 1.0;
  double zero = 0.0;
...
#pragma omp parallel for private(k, l, info, threadID, e)
      ...
      for(i = 0; i < n; i++){
        ...
        if (i > 0) {
          ...
          F[n*MeIndx+i] = sigmaSq - F77_NAME(ddot)(&nnIndxLU[n+i], &B[nIndx*MeIndx+nnIndxLU[i]], &inc, &c[twomp1*threadID], &inc); // change: B and F index
        } else {
          B[nIndx*MeIndx+i] = 0; // change: B index
          F[n*MeIndx+i] = sigmaSq; // change: F index
        }
      }
    } else {
#ifdef _OPENMP
      ...
      for(i = 0; i < n; i++){
        ...
        for(k = 0; k < nnIndxLUAll[n+i]; k++){
          if (k < m+1) {
            ...
            c[twomp1*threadID+k] = rho[MeIndx-1]*sigmaSq*
              spCor(e, phi, nu, covModel, &bk[threadID*nb]); // change: rho[MeIndx-1]
          } else {
            k2 = k-m-1;
            e = dist2(coords[i], coords[n+i],
            ...
            if (k < m+1) {
              ...
              if (l < m+1) {
                e = dist2(coords[nnIndx[nnIndxLU[i]+k2]],coords[n+nnIndx[nnIndxLU[i]+k2]],
                          coords[nnIndxParent[nnIndxLUParent[i]+l]],coords[n+nnIndxParent[nnIndxLUParent[i]+l]]);
                C[twomp1sq*threadID+l*nnIndxLUAll[n+i]+k] = rho[MeIndx-1]*sigmaSq*
                  spCor(e, phi, nu, covModel, &bk[threadID*nb]); // change: rho[MeIndx-1]
              } else {
                l2 = l-m-1;
                e = dist2(coords[nnIndx[nnIndxLU[i]+k2]],coords[n+nnIndx[nnIndxLU[i]+k2]],
        ...
        F[n*MeIndx+i] = sigmaSq - F77_NAME(ddot)(&nnIndxLUAll[n+i], &B[nIndx*MeIndx+nnIndxLUAll[i]], &inc, &c[twomp1*threadID], &inc); // change: B and F index                     
      }
    }
  }
}
