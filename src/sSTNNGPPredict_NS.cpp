...
extern "C" {
  ...
  SEXP sSTNNGPPredict_NS(SEXP X_r, SEXP y_r, SEXP coords_r, SEXP adjvec_r, 
    ...
    double *sigmaSq = REAL(sigmaSqSamples_r);                                   // BJ
    double *phi = REAL(phiSamples_r);                                           // BJ
    double *crossphi = REAL(crossphiSamples_r);                                 // BJ
    double *nu = REAL(nuSamples_r);                                             // BJ
    ...
    double phij, phik, crossphij, nuj = 0, nuk = 0, crossnu = 0, sigmaSqj, sigmaSqk, crosssigma, d;  // BJ
    ...
    for (MeIndx = 0; MeIndx < q; MeIndx++) {                                    // BJ
      ...
      if (MeIndx == 0) {                                                        // BJ: root
        for(i = 0; i < n0; i++){
#ifdef _OPENMP
#pragma omp parallel for private(threadID, phij, nuj, sigmaSqj, tauSqj, k, l, d, info) // BJ
#endif
          ...
          for(s = 0; s < nSamples; s++){
            ...
            phij = phi[s*q+MeIndx];                                             // BJ
            if(corName == "matern"){
              nuj = nu[s*q+MeIndx];                                             // BJ
            }
            sigmaSqj = sigmaSq[s*q+MeIndx];                                     // BJ
            tauSqj = tauSq[s*q+MeIndx];                                         // BJ
            
            for(k = 0; k < m; k++){
              d = dist2(coords[nnIndx0[i+n0*k]], coords[n+nnIndx0[i+n0*k]], coords0[i], coords0[n0+i]);
              c[threadID*twom+k] = sigmaSqj*spCor(d, phij, nuj, covModel, &bk[threadID*nb]);
              
              for(l = 0; l <= k; l++){
                d = dist2(coords[nnIndx0[i+n0*k]], coords[n+nnIndx0[i+n0*k]], coords[nnIndx0[i+n0*l]], coords[n+nnIndx0[i+n0*l]]);
                C[threadID*twomsq+l*m+k] = sigmaSqj*spCor(d, phij, nuj, covModel, &bk[threadID*nb]);
              }
            }
...
#pragma omp atomic
        ...
        for(i = 0; i < n0; i++){
#ifdef _OPENMP
#pragma omp parallel for private(threadID, phij, phik, crossphij, nuj, nuk, crossnu, sigmaSqj, sigmaSqk, crosssigma, tauSqj, rhoj, k, l, d, info) // BJ
#endif
          ...
          for(s = 0; s < nSamples; s++){
            ...
            phij = phi[s*q+MeIndx];                                             // BJ
            phik = phi[s*q+ParentIndx];                                         // BJ
            crossphij = crossphi[s*(q-1)+(MeIndx-1)];                           // BJ
            if(corName == "matern"){
              nuj = nu[s*q+MeIndx];                                             // BJ
              nuk = nu[s*q+ParentIndx];                                         // BJ
              crossnu = 0.5*nuj + 0.5*nuk;                                      // BJ
            }
            sigmaSqj = sigmaSq[s*q+MeIndx];                                     // BJ
            sigmaSqk = sigmaSq[s*q+ParentIndx];                                 // BJ
            crosssigma = pow(sigmaSqj,0.5)*pow(sigmaSqk,0.5);                   // BJ
            tauSqj = tauSq[s*q+MeIndx];                                         // BJ
            ...
            for(k = 0; k < twom; k++){
              if (k < m) {
                d = dist2(coords[nnIndx0[i+n0*k]], coords[n+nnIndx0[i+n0*k]], coords0[i], coords0[n0+i]);
                c[threadID*twom+k] = rhoj*crosssigma*spCor(d, crossphij, crossnu, covModel, &bk[threadID*nb]);
              } else {
                k2 = k-m;
              ...
              for(l = 0; l <= k; l++){
                if (k < m) {
                  d = dist2(coords[nnIndx0[i+n0*k]], coords[n+nnIndx0[i+n0*k]], coords[nnIndx0[i+n0*l]], coords[n+nnIndx0[i+n0*l]]);
                  C[threadID*twomsq+l*twom+k] = sigmaSqk*spCor(d, phik, nuk, covModel, &bk[threadID*nb]);
                } else{
                  k2 = k-m;
                  if (l < m) {
                    d = dist2(coords[nnIndx0[i+n0*k2]], coords[n+nnIndx0[i+n0*k2]], coords[nnIndx0[i+n0*l]], coords[n+nnIndx0[i+n0*l]]);
                    C[threadID*twomsq+l*twom+k] = rhoj*crosssigma*spCor(d, crossphij, crossnu, covModel, &bk[threadID*nb]);
                  } else{
                    l2 = l-m;
                    d = dist2(coords[nnIndx0[i+n0*k2]], coords[n+nnIndx0[i+n0*k2]], coords[nnIndx0[i+n0*l2]], coords[n+nnIndx0[i+n0*l2]]);
                    C[threadID*twomsq+l*twom+k] = sigmaSqj*spCor(d, phij, nuj, covModel, &bk[threadID*nb]);
                  }
                }
              }
            }
