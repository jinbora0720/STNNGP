...
#include "util.h"
#include "rpg.h"
#include "STNNGP.h"                                                             // BJ: for updateBF_parent_NS
...
extern "C" {
  ...
  SEXP sSTNNGPLogit_NS(SEXP y_r, SEXP X_r, 
    ...
    int *nTrial = INTEGER(nTrial_r);                                            // BJ
    ...
    if(verbose){
      ...
      Rprintf("----------------------------------------\n");
      Rprintf("Spanning Tree-Based NNGP Non-separable Latent model fit with %i Bernoulli observations.\n\n", n);
    ...
    double *kappa = (double *) R_alloc(nq, sizeof(double)); zeros(kappa, nq);  
    double *yStr = (double *) R_alloc(nq, sizeof(double)); zeros(yStr, nq); 
    ...
    double *tmp_np = (double *) R_alloc(np, sizeof(double)); zeros(tmp_np, n);
      ...
      for (MeIndx = 0; MeIndx < q; MeIndx++) {                                  // BJ
        ...
        for(i = 0; i < n; i++){
          if (missing[n*MeIndx+i] != 1.0) {
            omega[n*MeIndx+i] = rpg(nTrial[n*MeIndx+i],
                                    F77_NAME(ddot)(&p, &X[i], &n, &beta[p*MeIndx], &inc) + w[n*MeIndx+i]);
            yStr[n*MeIndx+i] = kappa[n*MeIndx+i]/omega[n*MeIndx+i];
          }
        }
        ...
        if (MeIndx == 0) {
          for(i = 0; i < n; i++){
            ...
            if (missing[n*MeIndx+i] == 1.0) {
              ...
              mu = (yStr[n*MeIndx+i]-F77_NAME(ddot)(&p, &X[i], &n, &beta[p*MeIndx], &inc))*omega[n*MeIndx+i] +
                e/F[n*MeIndx+i] + a + ac;
              var = 1.0/(omega[n*MeIndx+i] + 1.0/F[n*MeIndx+i] + v + vc);     // BJ
            }
        ...
        for(i = 0; i < n; i++){
          if (missing[n*MeIndx+i] == 1.0) {                                     // BJ
            ...
            tmp_n[i] = (yStr[n*MeIndx+i] - w[n*MeIndx+i])*omega[n*MeIndx+i];    // BJ
          }
        }
        ...
        for(i = 0; i < n; i++){
          for(j = 0; j < p; j++){
            tmp_np[j*n+i] = X[j*n+i]*omega[n*MeIndx+i];                         // BJ: will have zero for i's where MeIndx is not observed
          }
        }
...
#pragma omp parallel for private (e, j, b) reduction(+:a_th, logDet)
    ...
    SET_VECTOR_ELT(result_r, 1, sigmaSqSamples_r);
    SET_VECTOR_ELT(resultName_r, 1, mkChar("p.sigmasq.samples"));
    
    SET_VECTOR_ELT(result_r, 2, phiSamples_r);
    SET_VECTOR_ELT(resultName_r, 2, mkChar("p.phi.samples"));
    
    SET_VECTOR_ELT(result_r, 3, crossphiSamples_r);
    SET_VECTOR_ELT(resultName_r, 3, mkChar("p.crossphi.samples"));
