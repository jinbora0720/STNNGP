STNNGP_NS <- function(formula, data = parent.frame(), coords, method = "response", # BJ
  ...
  if(family == "gaussian"){
    ...
    if(ncol(tau.sq.IG) != 2 || nrow(tau.sq.IG) != q){                           # BJ
      stop("error: tau.sq.IG must be a q-by-2 matrix")                          # BJ
    }                                                                           # BJ
  ...
  if(family == "gaussian"){
    ...
    tau.sq.starting <- starting[["tau.sq"]]                                     # BJ
  ...
  out$p.sigmasq.samples <- mcmc(t(out$p.sigmasq.samples))
  out$p.phi.samples <- mcmc(t(out$p.phi.samples))
  out$p.crossphi.samples <- mcmc(t(out$p.crossphi.samples))
  out$p.tausq.samples <- mcmc(t(out$p.tausq.samples))
  out$p.rho.samples <- mcmc(t(out$p.rho.samples))
  if (cov.model == "matern") {
    out$p.nu.samples <- mcmc(t(out$p.nu.samples))
  }
